...
const ChangeShop = () => {
  ...
  const isNoShop = allShops?.length === 0
  ...
  return (
    <>
      <Tooltip
        title={isNoShop ? 'Add Shop' : 'Change Shop'}
      ...
      >
        <IconButton ref={anchorRef} onClick={handleToogleAdd} className='text-textPrimary'>
          <i className={`${isNoShop ? 'tabler-building-store' : 'tabler-arrows-exchange'}`} />
        </IconButton>
      </Tooltip>
      <Popper
        open={open}
              ...
              <ClickAwayListener onClickAway={handleClose}>
                <MenuList onKeyDown={handleClose} className='max-h-[200px] overflow-y-auto w-[200px] overflow-x-clip'>
                  {allShops?.map(shopDetails => (
                    ...
                    >
                      {shopDetails.name.length > 20 ? `${shopDetails.name.substring(0, 20)}...` : shopDetails.name}
                    </MenuItem>
                  ))}
                </MenuList>
              </ClickAwayListener>
