  ...
  if (data.storeCategory != 'F&B') {
    ...
    if (!data?.pid) {
      ...
      const response = await axiosInstance.patch(
        `/ironcore/product-api/v0/products/${data.pid}/?store_id=${data.storeId}`,
        apibody
      )
...
const useSubmitProduct = () => {
  const { allShops, variantData, variantDataId } = useEssentialDataStore(state => state)
  const { setVariantDataId } = useEssentialDataStore()
  const store = allShops?.[1]
  const storeCategory = store?.category?.category_detail?.name
  const storeId = store?.id

  // const storeId = 'bd05b8aa-791e-4f51-9aac-b69af4146e35'
  // const storeCategory = 'F&B'
  const storeTime = store?.time
  const storeTimeToShip = store?.category?.time_to_ship
