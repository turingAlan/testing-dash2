...
const ProductPage = ({ pid }: { pid?: string | number }) => {
  const { allShops, categoryMetaData, variantData } = useEssentialDataStore(state => state)
  const { setVariantData } = useEssentialDataStore()
  const currentStore = allShops?.[1]

  // currentStore['id'] = 'bd05b8aa-791e-4f51-9aac-b69af4146e35'

  // Use optional chaining and nullish coalescing to safely access the properties
  const currCategory = currentStore?.category?.category_detail?.name ?? ''

  // const currCategory = 'F&B'

  const subcategories = categoryMetaData?.data[currCategory].sub_categories
  ...
  const getFoodSubcategories = () => {
    ...
    axiosInstance
      .get(`/ironcore/product-api/v0/food-products/get_categories/?store_id=${currentStore?.id}`)
      .then(res => {
  ...
  const getFoodCustomizations = () => {
    if (currCategory != 'F&B' && !selectedSubcategory?.id) return
    axiosInstance
                          ...
                          <div
                            ...
                            onClick={() => {
                              if (addSubcategory) {
                                axiosInstance
                                  .post(
                                    `/ironcore/product-api/v0/food-products/create_category/?store_id=${currentStore?.id}`,
                                    { name: addSubcategory }
                                  )
                                  .then(() => {
