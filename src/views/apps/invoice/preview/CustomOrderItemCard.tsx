            ...
            return (
              <div className='flex items-center gap-3'>
                {thumbnailImage?.image ? (
                  <img src={thumbnailImage?.image} alt={row.original.product.name} height={34} />
                ) : (
                  <img src={'/images/illustrations/characters/5.png'} alt={row.original.product.name} height={34} />
                )}
                <div>
                  <Typography
                    component={Link}
        ...
        columnHelper.accessor('product.id', {
          ...
          cell: ({ row }) => {
            ...
            const orderItemCancellable =
              !orderItemCancelled &&
              !orderItemEmpty &&
              (fulfillmentData?.delivery_status === 'Pending' || fulfillmentData?.delivery_status === 'Packed')
        ...
        <div className='m-4 flex flex-wrap space-x-2 space-y-1 items-center'>
          {orderItems.length > 0 && (
            <>
              ...
              {currentShopData.fulfillment === deliveryTypes.SELF &&
                fulfillmentData?.delivery_status !== orderStatus.ORDER_DELIVERED && (
...
const OderTableRto: React.FC<OrderTableProps> = ({ fulfillmentData }) => {
  const { lang: locale, order_id } = useParams()
      ...
      return [
        ...
        rtoColumnHelper.accessor('product.id', {
          ...
          cell: ({ row }) => {
            ...
            return (
              <Button variant='outlined' color='primary' disabled={!orderItemCancellable}>
                <Typography>{`${orderItemCancellable ? 'Cancel this item' : orderStatus.CANCELLED}`}</Typography>
              </Button>
            )
          }
  ...
  return (
    <>
      <OrderCancelDialog
        open={orderCancelItemPopup}
        setOpen={setOrderCancelItemPopup}
        handleOnConfirmation={(reason: string, quantity: number) => {
          console.log(reason, quantity)
        }}
      />
      <div className='overflow-x-auto'>
        <table className={tableStyles.table}>
